...
export default function EndpointStatusPanel() {
  ...
  const fetchTriage = async () => {
    try {
      ...
      const latest = data.events?.find(e => e.type === "ENDPOINT_TRIAGE");
  ...
  useEffect(() => {
    fetchTriage();
    const interval = setInterval(fetchTriage, 60000); // Refresh every minute
  ...
  if (loading) {
    return (
      <div className="p-4 bg-white/10 rounded-lg border border-gray-300 shadow">
        <h3 className="text-lg font-semibold mb-2">🩺 Endpoint Triage</h3>
        <p className="text-sm text-gray-500">Fetching endpoint triage...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-white/10 rounded-lg border border-gray-300 shadow">
      ...
      {triage.meta?.failedEndpoints && triage.meta.failedEndpoints.length > 0 && (
        <div className="mt-3 p-2 bg-red-50 rounded border border-red-200">
          <p className="text-xs font-semibold text-red-800 mb-1">Failed Endpoints:</p>
          <ul className="list-disc ml-4 text-xs text-red-700">
            {triage.meta.failedEndpoints.map((endpoint) => (
      ...
      {triage.meta?.results && (
        <div className="mt-3">
          <p className="text-xs font-semibold mb-1 text-gray-700">Endpoint Details:</p>
          <div className="space-y-1">
            {triage.meta.results.map((result) => (
